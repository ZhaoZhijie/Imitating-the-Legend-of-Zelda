...
class CMove
{
	movep m_Move[64];	//动作 （图片 默认64张 一个动作最大64张图片表现）
	int m_Length;		//该动作长度
...
public: 
	...
	//添加动作帧(也就是添加动作的图片)
	void AddFrame(char *pid, float x1, float y1, float x2, float y2, int CF, float x, float y, int site, int bigx, int bigy);
	//添加特效
...
public:
	...
	bool AddFrame(int id, char *pid, float x1, float y1, float x2, float y2, int CF, float x, float y, int site = 5, int bigx = 0, int bigy = 0);
