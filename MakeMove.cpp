...
//添加动作帧
void CMove::AddFrame(char *pid, float x1, float y1, float x2, float y2, int CF, float x, float y, int site, int bigx, int bigy)
{
...
////得到补正
//float CMove::GetX(int f)
//{
//	return m_Move[f]._x;
//}
//float CMove::GetY(int f)
//{
//	return m_Move[f]._y;
//}
...
bool CMoves::AddFrame(int id, char *pid, float x1, float y1, float x2, float y2, int CF, float x, float y, int site, int bigx, int bigy)
{
	if(id < m_movLen || id > m_movMax)
		return false;
	m_Moves[id].AddFrame(pid, x1, y1, x2, y2, CF, x, y, site, bigx, bigy);
	return true;
}
